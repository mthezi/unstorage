...
**Type-safe storage with constants pattern:**
...
// Create typed storage definition using constants as keys
type MyAppStorage = {
  items: {
...
// Create storage with automatic type inference
const storage = createStorage<MyAppStorage>();

// Now getItem automatically infers the correct return type
const theme = await storage.getItem(STORAGES.APP_THEME_STATE); // ThemeState | null
const prefs = await storage.getItem(STORAGES.USER_PREFERENCES); // UserPreferences | null
const session = await storage.getItem(STORAGES.SESSION_DATA); // SessionData | null

// setItem enforces correct types
await storage.setItem(STORAGES.APP_THEME_STATE, {
  mode: "dark",
  primaryColor: "#007acc",
}); // ✅ Type-safe

await storage.setItem(STORAGES.APP_THEME_STATE, "invalid"); // ❌ TS Error

// setItems also supports type-safe batch operations
await storage.setItems([
  ...
  },
  {
    ...
    value: { language: "zh", notifications: false, autoSave: true },
  },
]); // ✅ Each value is type-checked based on its key

// await storage.setItems([
//   { key: STORAGES.APP_THEME_STATE, value: 'invalid' } // ❌ TS Error
// ]);

// getItems also provides type-safe batch retrieval
const items = await storage.getItems([
  STORAGES.APP_THEME_STATE,
  STORAGES.USER_PREFERENCES,
]);
// items: { key: string; value: ThemeState | UserPreferences | null }[]

// Each item's value is correctly typed based on its key
items.forEach((item) => {
  if (item.key === STORAGES.APP_THEME_STATE) {
    // item.value is ThemeState | null
    console.log("Theme mode:", item.value?.mode);
  } else if (item.key === STORAGES.USER_PREFERENCES) {
    // item.value is UserPreferences | null
    console.log("Language:", item.value?.language);
  }
});
```
